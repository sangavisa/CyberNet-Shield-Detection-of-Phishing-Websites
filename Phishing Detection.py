...
def web_traffic(url):
    try:
        ...
        # Fetch the data from Alexa using the provided URL
        api_url = f"http://data.alexa.com/data?cli=10&dat=s&url={url}"
        response = urllib.request.urlopen(api_url)

        # Parse the XML data using BeautifulSoup
        soup = BeautifulSoup(response.read(), "xml")

        # Find the 'REACH' tag and extract the 'RANK' attribute
        reach = soup.find("REACH")
        if reach:
            rank = reach['RANK']
            rank = int(rank)
        else:
            return 1  # Return 1 if no reach data is found
    ...
    if rank < 100000:
        return 1  # Website has good traffic (rank < 100,000)
    else:
        return 0 
